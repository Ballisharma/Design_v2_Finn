...
const ProductDetails: React.FC<ProductDetailsProps> = ({ isModal = false }) => {
  ...
  const handleAddToCart = () => {
    if (selectedSize && maxStockForSelection > 0) {
      ...
      // Optional: Add simple mobile feedback if needed, though cart drawer usually handles it
    }
  };
             ...
             <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {product.variants.map((v) => (
                   <button
                     ...
                     disabled={v.stock === 0}
                     className={`min-w-[4.5rem] py-3.5 px-4 rounded-2xl font-bold text-sm transition-all border-2 relative ${
                        selectedSize === v.size 
       ...
       <div className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-xl border-t border-gray-200 p-4 z-40 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
          <div className="flex gap-3 max-w-lg mx-auto">
             ...
             <button 
               ...
               className={`flex-1 py-4 rounded-2xl font-heading font-black text-lg shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 ${
                 isSizeOutOfStock 
              ...
              <div className="flex gap-3 flex-wrap">
                  {product.variants.map((v) => (
                    <button
                      ...
                      onClick={() => { setSelectedSize(v.size); setQuantity(1); }}
                      disabled={v.stock === 0}
                      className={`px-6 py-3 rounded-lg font-bold border-2 transition-all relative ${
                          selectedSize === v.size 
